...
@Component({
  selector: 'app-view-admin-email-template',
  templateUrl: './view-admin-email-template.component.html',
  styleUrls: ['./view-admin-email-template.component.css']
})
export class ViewAdminEmailTemplateComponent implements OnInit {
  ...
  getForgetPasswordTemplate(filters) {
    ...
    this.emailConfigurationService.getForgetPasswordTemplate(filters).subscribe( response =>{
      ...
      this.assignData  = response?.data?.docs[0]?.admin_email_template;
  ...
  openModal(mode: Mode, id = null, item = null) {
    this.modal = this.modalService.create({
      ...
      nzContent: AddAdminEmailTemplateComponent,
      nzFooter: [
        {
