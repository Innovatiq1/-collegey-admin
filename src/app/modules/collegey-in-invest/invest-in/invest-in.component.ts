...
@Component({
  ...
  styleUrls: ['./invest-in.component.css'],
})
export class InvestInComponent implements OnInit {
  mode: String;
  id: any;
  itemData: any;
  pageContent: any;
  investInForm: FormGroup;

  selectFile:any;

  constructor(
    private fb: FormBuilder,
    private investService: InvestInService,
  ...
  createForm() {
    this.investInForm = this.fb.group({
    ...
    this.investInForm.valueChanges.subscribe(() => {});
  }
  ...
  save() {
    return new Promise<void>((resolve, reject) => {
      this.markAllTouched();
      if (this.investInForm.valid) {
        if (this.mode === Mode.Create) {
          let obj = this.investInForm.value;
          obj['selectFile']   = this.selectFile;
          this.investService.createTheamData(obj).subscribe(
            (res) => {
              this.modal.destroy();
              resolve();
              Swal.fire({
          ...
          let obj = this.investInForm.value;
          obj['selectFile']   = this.selectFile;
          this.investService.updateTheamData(obj, this.id).subscribe(
            (res) => {
              Swal.fire({
                title: 'Successful',
  ...
  markAllTouched() {
    for (const i in this.investInForm.controls) {
      this.investInForm.controls[i].markAsDirty();
      this.investInForm.controls[i].updateValueAndValidity();
    }
  }
