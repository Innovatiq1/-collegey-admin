...
@Component({
  selector: 'app-viewmentor-resources',
  templateUrl: './viewmentor-resources.component.html',
  styleUrls: ['./viewmentor-resources.component.css'],
})
export class ViewmentorResourcesComponent implements OnInit {
  searchText: string = '';
  resourceData = [];
  isLoading = false;
  modal: NzModalRef;
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe((params) => {
      this.getMentorResourcesList(params);
    });
  }

  getMentorResourcesList(filters) {
    this.show_loader = true;
    this.mentorService.getMentorResourcesList(filters).subscribe(
      (response) => {
  ...
  openModal(mode: Mode, id = null, item = null) {
    this.modal = this.modalService.create({
      nzTitle:
        mode === 'Create' ? 'Create Mentor Resource' : 'Update Mentor Resource',
      nzContent: MentorResourcesComponent,
      nzFooter: [
        {
  ...
  deleteResource(listid: any) {
    if (confirm('Are you sure to delete this resource')) {
      ...
      this.mentorService.deleteMentorResource(mylist).subscribe((response) => {
