...
@Component({
  selector: 'app-list-agreement-terms',
  templateUrl: './list-agreement-terms.component.html',
  styleUrls: ['./list-agreement-terms.component.css']
})
export class ListAgreementTermsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe(params => {
      this.getAgreementTerms(params);
    });
  }

  getAgreementTerms(filters) {
    this.show_loader = true;
    let type_policy = 'mentor';
    this.mentorService.getAgreementTermsList(filters,type_policy).subscribe(response => {
  ...
  openModal(mode: Mode, id = null, item = null) {
    this.modal = this.modalService.create({
      nzTitle: mode === "Create" ? "Create Aggrement" : "Update Aggrement",
      nzContent: AgreementTermsConditionComponent,
      nzFooter: [
        {
          label: mode === "Create" ? "Create" : "Update",
          show: item ? (item.isDeleted ? true : true) : true,
          type: "primary",
