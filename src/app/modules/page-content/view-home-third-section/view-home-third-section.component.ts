...
@Component({
  selector: 'app-view-home-third-section',
  templateUrl: './view-home-third-section.component.html',
  styleUrls: ['./view-home-third-section.component.css']
})
export class ViewHomeThirdSectionComponent implements OnInit {
  ...
  getHomeFirstSecData(filters) {
    ...
    this.pageContentService.getHomeFirstSecData(filters).subscribe(response => {
      ...
      this.assignData = response?.data?.docs[0]?.home_third_section;
  ...
  openModal(mode: Mode, id = null, item = null, type: any = null) {
    ...
    if (type === "mainTitle") {
      if (mode === "Create") {
        title = "Home Third Section - Add Data"
      } else { title = "Home Third Section - Update Data" }
    } else {
      if (mode === "Create") {
        title = "Home Third Section - Add Video"
      } else { title = "Home Third Section - Update Video" }
    }
    this.modal = this.modalService.create({

      nzTitle: title,
      nzContent: AddHomeThirdSectionComponent,
  ...
  deleteHome(index: any, type: any) {
    if (confirm("Are you sure to delete this item")) {
      ...
      var deletelist = { index: index, filedName: "home_third_section", type: type };
