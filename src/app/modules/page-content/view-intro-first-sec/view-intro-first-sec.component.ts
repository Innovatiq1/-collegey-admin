...
@Component({
  selector: 'app-view-intro-first-sec',
  templateUrl: './view-intro-first-sec.component.html',
  styleUrls: ['./view-intro-first-sec.component.css']
})
export class ViewIntroFirstSecComponent implements OnInit {
  ...
  getHomeFirstSecData(filters) {
    ...
    this.pageContentService.getHomeFirstSecData(filters).subscribe(response => {
      ...
      this.assignData = response?.data?.docs[0]?.home_second_section;
  ...
  openModal(mode: Mode, id = null, item = null, type: any = null) {
    ...
    if (type === "mainTitle") {
      if (mode === "Create") {
        title = "Home Second Section - Add Title"
      } else { title = "Home Second Section - Update Title" }
    } else {
      if (mode === "Create") {
        title = "Create Sub Title"
      } else { title = "Update Sub Title" }
    }
    this.modal = this.modalService.create({

      nzTitle: title,
      nzContent: AddIntroFirstSecComponent,
  ...
  deleteHome(index: any, type: any) {
    if (confirm("Are you sure to delete this item")) {
      ...
      var deletelist = { index: index, filedName: "home_second_section", type: type };
