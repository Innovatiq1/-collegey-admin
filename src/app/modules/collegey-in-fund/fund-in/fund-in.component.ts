...
@Component({
  selector: 'app-fund-in',
  templateUrl: './fund-in.component.html',
  styleUrls: ['./fund-in.component.css'],
})
export class FundInComponent implements OnInit {
  mode: String;
  id: any;
  itemData: any;
  pageContent: any;
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (this.fundInForm.valid) {
        if (this.mode === Mode.Create) {
          ...
          this.fundService.createTheamData(obj).subscribe(
            (res) => {
              this.modal.destroy();
              resolve();
              Swal.fire({
          ...
          let obj = this.fundInForm.value;
          obj['selectFile'] = this.selectFile;
          this.fundService.updateTheamData(obj, this.id).subscribe(
            (res) => {
              Swal.fire({
                title: 'Successful',
