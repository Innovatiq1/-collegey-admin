...
@Component({
  ...
  styleUrls: ['./add-alumni.component.css']
})
export class AddAlumniComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AddAlumniComponent>,
  ...
  addBlog(formObj):any {
    return new Promise((resolve, reject) => {
      this.alumniService.saveAlumni(formObj).subscribe(
        (response) => {
  ...
  updateBlog(obj):any {
    return new Promise((resolve, reject) => {
      this.alumniService.updateAlumni(obj, this.mentor._id).subscribe(
        (response) => {
          this.isLoading = false;
          Swal.fire({
            title: 'Successful',
            text: 'Alumni updated succesfully',
            icon: 'success',
          }).then(() => {
             resolve(response);
          });
