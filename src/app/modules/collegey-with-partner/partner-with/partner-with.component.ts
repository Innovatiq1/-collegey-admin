...
@Component({
  selector: 'app-partner-with',
  templateUrl: './partner-with.component.html',
  styleUrls: ['./partner-with.component.css']
})
export class PartnerWithComponent implements OnInit {
  mode: String;
  id: any;
  itemData: any;
  pageContent: any;
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      if (this.fundInForm.valid) {
        if (this.mode === Mode.Create) {
          ...
          this.partnerWithService.createTheamData(obj).subscribe(
            (res) => {
              this.modal.destroy();
              resolve();
              Swal.fire({
          ...
          let obj = this.fundInForm.value;
          obj['selectFile'] = this.selectFile;
          this.partnerWithService.updateTheamData(obj, this.id).subscribe(
            (res) => {
              Swal.fire({
                title: 'Successful',
