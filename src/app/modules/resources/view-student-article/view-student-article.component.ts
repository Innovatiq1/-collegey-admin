...
@Component({
  selector: 'app-view-student-article',
  templateUrl: './view-student-article.component.html',
  styleUrls: ['./view-student-article.component.css']
})
export class ViewStudentArticleComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe(params => {
      this.getStudentArticleList(params);
    });
  }

  getStudentArticleList(filters) {
    this.isLoading   = true;
    this.show_loader = true;
    this.studentService.getStudentArticleList(filters).subscribe( response =>{
  ...
  openModal(mode: Mode, id = null, item = null) {
    this.modal = this.modalService.create({
      nzTitle: mode === "Create" ? "Create Student Article" : "Update Student Article",
      nzContent: AddStudentArticleComponent,
      nzFooter: [
        {
  ...
  deleteArticle(listid:any)
  {
    if(confirm("Are you sure to delete this artical"))
    {
      ...
      this.studentService.deleteStudentArticle(mylist).subscribe(
        (response)=> {
