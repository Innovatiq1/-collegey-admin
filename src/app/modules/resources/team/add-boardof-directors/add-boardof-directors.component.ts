...
@Component({
  selector: 'app-add-boardof-directors',
  templateUrl: './add-boardof-directors.component.html',
  styleUrls: ['./add-boardof-directors.component.css']
})
export class AddBoardofDirectorsComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddBoardofDirectorsComponent>,
  ...
  updateTeam(formData) {
    return new Promise((resolve, reject) => {
      this.teamService.updateBoardofDirectors(formData, this.team._id).subscribe(
        (team) => {
  ...
  saveTeam(formData) {
    return new Promise((resolve, reject) => {
      this.teamService.createBoardofDirectors(formData).subscribe(
        (team) => {
          Swal.fire('Successful', 'Board of Directors member added succesfully', 'success');
          this.isLoading = false;
          // this.dialogRef.close(conference);
          resolve(team)
        },
        (err) => {
          this.isLoading = false;
          Swal.fire('Failed to add board of directors member', err.message || err.error, 'error');
          reject(err)
        }
      );
    })
